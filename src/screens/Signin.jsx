...
function Signin() {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      if (err.response?.status === 429 || err.response?.data?.code === 'MAX_ATTEMPTS_REACHED') {
        setIsLocked(true);
        setOtp("");
        setOtpError(true);
        setLockTimeLeft(900); // 15 minutes in seconds
        toast.error("Maximum login attempts reached. Please try again after 15 minutes.");
        return;
      }
      
      // Handle invalid OTP with remaining attempts
      if (err.response?.data?.code === 'INVALID_OTP') {
        const remaining = err.response?.data?.remainingAttempts || 0;
        setRemainingAttempts(remaining);
        setOtp("");
        setOtpError(true);
        setShake(true);
        setTimeout(() => setShake(false), 500);
        toast.error(`Invalid OTP. ${remaining} attempt(s) remaining.`);
        return;
      }
      
      toast.error(err.response?.data?.error || "Error verifying OTP. Please try again.");
      setOtpError(true);
      setShake(true);
      setTimeout(() => setShake(false), 500);
    } finally {
              ...
              <div className="flex flex-col items-center space-y-6">
                ...
                <div className="w-full">
                  ...
                  {!isLocked && remainingAttempts < 5 && (
                    <motion.div 
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="text-center text-xs text-amber-600 mb-3 font-medium"
                    >
                    ...
                    {Array.from({ length: 6 }).map((_, index) => (
                      <input
                        ...
                        className={`w-10 h-12 md:w-12 md:h-14 text-lg md:text-xl font-semibold bg-white 
                          ...
                          ${isLocked ? 'border-gray-200 bg-gray-50 text-gray-400 cursor-not-allowed' : otpError ? 'border-red-400 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-blue-500 focus:ring-blue-500'} 
                ...
                <motion.button
                  ...
                  className={`w-full py-3 px-4 text-white rounded-lg font-medium transition-all duration-300 flex items-center justify-center gap-2 ${
                    loading || otp.length !== 6 || isLocked
                      ? "bg-gray-400 cursor-not-allowed"
                      : "bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 shadow-md hover:shadow-lg"
                  }`}
                  whileHover={{ scale: (otp.length === 6 && !loading && !isLocked) ? 1.03 : 1 }}
                  whileTap={{ scale: (otp.length === 6 && !loading && !isLocked) ? 0.98 : 1 }}
                >
                  {isLocked ? (
                    <>
                      <span>Account Locked - Try Again in {formatTime(lockTimeLeft)}</span>
                    </>
                  ) : loading ? (
                    <>
                      <motion.div 
